...
void PuppiProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    // std::cout << "PUPPI stuff: " << lWeights.size() << ", " << lAlphas.size() << ", " << lRawAlphas.size() << ", " << lNAlgos << std::endl;
    // // std::auto_ptr<edm::ValueMap<float> > lPupAlphaOut(new edm::ValueMap<float>());
    // std::vector < std::vector<double> > lPupAlphaOut;
    // for (int i = 0; i < lNAlgos; i++){
    //     lPupAlphaOut.push_back( new edm::ValueMap<float>() );
    // }
    ...
    for ( auto i0 = hPFProduct->begin(),
        ...
        reco::PFCandidate pCand( i0->charge(),i0->p4(),id );
